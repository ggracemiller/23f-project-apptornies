...
@employees.route('employees/names', methods=['GET'])
def get_employees_names():
    # get a cursor object from the database
    cursor = db.get_db().cursor()

    # use cursor to query the database for a list of products
    cursor.execute('SELECT employee_id, CONCAT(first_name, " ", last_name) AS name FROM employee')
...
@employees.route('attorneys/names', methods=['GET'])
def get_attorneys_names():
    # get a cursor object from the database
    cursor = db.get_db().cursor()

    # use cursor to query the database for a list of products
    cursor.execute('SELECT email, phone_number, hourly_rate, \
                   CONCAT(first_name, " ", last_name) AS name FROM employee\
                   WHERE employee_type = \'attorney\'')
